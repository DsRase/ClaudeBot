...
- **SQLite + Alembic** — долговременная память пользователей, их персональные настройки (выбранная модель, лимит контекста и т.п.). Миграции накатываются **автоматически при старте** (`tg_main.py` → `run_migrations()`).
...
```text
├── tg_main.py             # entrypoint TG-интерфейса: миграции + запуск поллинга
├── alembic.ini            # конфиг миграций
├── migrations/            # ревизии Alembic
├── config.example.yaml    # пример конфига бота
├── .env.example           # пример переменных окружения
├── docker/
│   └── telegram.Dockerfile  # Dockerfile для TG-интерфейса (копирует tg_main.py → main.py)
├── docker-compose.yml
├── resources/bot/img/     # статические ассеты бота
├── src/
│   ├── agent/             # interface-agnostic ядро (LangChain-агент)
│   │   ├── service.py     # ChatService: record_message / respond
│   │   ├── dto/           # IncomingMessage и т.п.
│   │   ├── ports/         # Protocol-интерфейсы (PermissionRequester, ResponseChannel, ThinkingIndicator)
│   │   ├── permissions/   # in-memory state + reset_session_permissions
│   │   ├── llm/           # ask, langTools, modelSelector
│   │   └── tools/         # тулы агента (fetch, search, memory, adaptive)
│   ├── bot/               # aiogram-слой (TG-интерфейс)
│   │   ├── adapters/      # TG-реализации портов из src/agent/ports/
│   │   ├── handlers/      # тонкие хендлеры команд и сообщений
│   │   ├── markups/       # inline-клавиатуры
│   │   ├── permissions/   # admin_required (TG-декоратор)
│   │   ├── utils/         # TG-specific утилиты (split, addThinkLoad)
│   │   └── router.py      # include_routers
│   ├── config/            # settings, загрузка yaml/env
│   ├── storage/
│   │   ├── redis/         # клиент и обёртки над Redis
│   │   ├── sqlite/        # SQLAlchemy-модели и репозитории
│   │   └── schemas/       # Pydantic DTO (ChatMessage и т.п.)
│   └── utils/
│       ├── logger/        # фабрика логгера (подавлен шум от Alembic)
│       └── messager/      # interface-agnostic форматирование
└── tests/                 # зеркалит src/, + tests/integrate/
```

### Разделение интерфейс ↔ ядро

`src/agent/` — interface-agnostic. Не знает ни про aiogram, ни про Telegram. Общается с интерфейсом через **порты** (Protocol):

| Порт                 | Что делает                                          | TG-реализация                                  |
|----------------------|-----------------------------------------------------|------------------------------------------------|
| `PermissionRequester`| Запрашивает у юзера разрешение на тул и ждёт ответа.| `TelegramPermissionRequester` (inline kbd + Event)|
| `ResponseChannel`    | Отправляет ответ или ошибку юзеру.                  | `TelegramResponseChannel` (telegramify_markdown + split)|
| `ThinkingIndicator`  | Async context manager «думаю…».                     | `TelegramThinkingIndicator` (think_msg + add_think_load)|

Чтобы добавить новый интерфейс (Discord, web):

1. Создаёшь `src/<iface>/adapters/` с реализациями трёх портов.
2. Создаёшь хендлеры/роуты, которые строят `IncomingMessage` и зовут `agent.service.record_message` / `respond`.
3. Создаёшь `<iface>_main.py` в корне.
4. Создаёшь `docker/<iface>.Dockerfile` (копирует свой main как `main.py`) и сервис в `docker-compose.yml`.
5. Никакого HTTP между интерфейсом и агентом — всё работает в одном процессе на каждый интерфейс.
...
В контейнере миграции применяются автоматически при старте (`tg_main.py`).
...
3. Зарегистрируй сборку в `src/agent/llm/langTools.py` (или подключи в `ChatService.respond` напрямую, если нужен chat_id/user_id-скоуп).
4. Если инструмент делает что-то потенциально вредное (I/O, сеть, запись файлов) — `ask()` сам спросит разрешение через `PermissionRequester`-порт.
